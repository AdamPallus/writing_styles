...
library(stringi)
z<-readLines('slowregard.txt',encoding="UTF-8",skipNul=TRUE)
...
dataset <- map(
  seq(1, length(text) - maxlen - 1, by = 2), 
...
{
  sink('SlowRegard256-64drop-1.txt')
  for(diversity in c(0.005, 0.05, 0.1,0.2, 0.4, 0.6,0.8, 1)){
  # for(diversity in c(.04)){#c(0.005,.1,.4,.6,.8,1)){
...
save_model_hdf5(model,'SlowRegard256-64.hd5') 
saveRDS(text,'slowregard256-64text.RDS') 
saveRDS(chars,'slowregard256-64chars.RDS')
...
#slow regard 13 is two 256 layers trained with lower case only and sentencce length 50
