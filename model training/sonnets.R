...
library(stringi)
z<-readLines('sonnets.txt',encoding="UTF-8-BOM",skipNul=TRUE)
# z<-stri_trans_general(z, "latin-ascii")

# z<- z[-1]

z<-str_replace_all(z,"[1234567890]","")
# 
...
model %>%
  # layer_lstm(1024, input_shape = c(maxlen, length(chars)),return_sequences = TRUE) %>%
...
{
sink('TrainingEE128-128drop.txt',append=TRUE)
...
save_model_hdf5(model,'Sonnets1a.hd5') 
saveRDS(text,'Sonnets1text.RDS')
saveRDS(chars,'Sonnets1chars.RDS')
